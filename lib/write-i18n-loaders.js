...
const createIntlLoader = (loaderName, locale, polyfill, format) => {
  if (format === 'es6') {
    return (
      `const ${loaderName} = () =>
   import('@formatjs/intl-${polyfill}/locale-data/${locale}.js')
...
`
  ...
  return require.ensure(['@formatjs/intl-${polyfill}/locale-data/${locale}.js'], function (require) {},  function (error) {
    console.log('An error occurred while loading @formatjs/intl-${polyfill}/locale-data/${locale}' + "\\n" + error);
...
const writeLoaders = (type, locales, fileSystem, outputDir, format) => {
  ...
  locales.forEach((locale) => {
    ...
    if (type === 'intl') {
      loaders[`'${locale}'`] = {};
      ['relativetimeformat', 'numberformat', 'datetimeformat'].forEach((polyfill) => {
      ...
      // Plural rules does not have regional locale data
      if (locale.split('-').length === 1) {
        const loaderName = `loadPluralrules${localeLoaderName}${startCase(type)}`;
        loaders[`'${locale}'`]['pluralrules'] = loaderName;
        loaderFile += createIntlLoader(loaderName, locale, 'pluralrules', format);
      }
    } else {
